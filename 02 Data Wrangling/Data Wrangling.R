...
# select
select(df, cut, clarity) %>% tbl_df # Equivalent SQL: select cut, clarity from diamonds;
...
df %>% mutate(ntile_z = ntile(facebook,100)) %>% arrange(desc(ntile_z)) %>% tbl_df
...
df %>% mutate(ntile_facebook = ntile(facebook,100)) %>% arrange(desc(ntile_facebook)) %>% tbl_df
df %>% mutate(ntile_twitter = ntile(twitter,100)) %>% arrange(desc(ntile_twitter)) %>% tbl_df

df %>% mutate( sum(facebook,twitter)) %>% tbl_df()

fb <- df %>% select(facebook) 
tr <- df %>% select(twitter) 

as.numeric(fb)
tr

total <- sum(as.numeric(fb),as.numeric(tr))
 
sum(df$facebook, df$twitter)
as.numeric(df$facebook)[1]
df$facebook[1]
df %>% mutate(totalFollowers = as.numeric(facebook) + as.numeric(twitter))
